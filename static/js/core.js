    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.error) {
                ...
                //} else {
                    // $("#dl_beta").attr("href", data['apple_url']);
                    //$("#dl_beta").attr("onclick", "window.open('"+data['apple_url']+"')");
                //}
            }
        }
    });
        ...
        success: function (data) {
            if (data.error) {
                ...
                //} else {
                    ...
                    //$("#downloadlink").attr("onclick", "window.open('"+data['url']+"')");
                //}
...
function install_profile($model) {
    console.log($model.indexOf("db"))
    if ($model.indexOf("db") == -1 && $model.indexOf('noota') == -1) {
        alert("正式版已经发布了，去设置里检查更新升级，或者在下方下载固件升级吧～")
        return;
    } else if ($model.indexOf("notnow") != -1) {
        console.log("notnow")
        alert("此版本仍未发布，但快了，请再等一等。")
        return;
    }

    // 慢慢的拼凑 拼凑成一个完全不属于自己的我
    var url = "http://" + window.location.host + "/Install/" + $model;
    if ($model == "tvOS13db" || $model == "tvOS14db") {
